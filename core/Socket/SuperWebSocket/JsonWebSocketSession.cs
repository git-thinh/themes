...
namespace SuperWebSocket
{
        ...
        where TWebSocketSession : JsonWebSocketSession<TWebSocketSession>, new()
    {
        ...
        {
            ///???????????????????????????????????????????
            //return string.Format(m_QueryTemplate, name, Newtonsoft.Json.JsonConvert.SerializeObject(content));
            return string.Format(m_QueryTemplate, name, System.Json.Converter.Serialize(content));
        }
