...
namespace SuperWebSocket
{
        ...
        where TWebSocketSession : WebSocketSession<TWebSocketSession>, new()
    {
        ...
        private ConcurrentPriorityQueue<TWebSocketSession> m_OpenHandshakePendingQueue = new ConcurrentPriorityQueue<TWebSocketSession>();

        private ConcurrentPriorityQueue<TWebSocketSession> m_CloseHandshakePendingQueue = new ConcurrentPriorityQueue<TWebSocketSession>();
        ...
        private void HandshakePendingQueueCheckingCallback(object state)
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    //if (!m_OpenHandshakePendingQueue.TryPeek(out session)) break;
                    session = m_OpenHandshakePendingQueue.Peek().Value;
                    ...
                    if (session.Handshaked || session.Status != SessionStatus.Healthy)
                    {
                        ...
                        session = m_OpenHandshakePendingQueue.Dequeue().Value;
                        continue;
                    }
                ...
                while (true)
                {
                    ...
                    //if (!m_CloseHandshakePendingQueue.TryPeek(out session)) break;
                    session = m_CloseHandshakePendingQueue.Peek().Value;
                    if (session == null) break;
                    ...
                    if (session.Status != SessionStatus.Healthy)
                    {
                        ...
                        session = m_CloseHandshakePendingQueue.Dequeue().Value;
                        continue;
                    }
