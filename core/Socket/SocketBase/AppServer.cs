        ...
        where TAppSession : IAppSession<TAppSession, TCommandInfo>, new()
    {
        ...
        internal protected override void OnSocketSessionClosed(object sender, SocketSessionClosedEventArgs e)
        {
            ...
            if (m_SessionDict.TryRemove(identityKey, out removedSession))
            {
                ...
                Async.Run(() => OnAppSessionClosed(this, new AppSessionClosedEventArgs<TAppSession>(removedSession, e.Reason)), exc => Logger.LogError(exc));

                //SmartThreadPool smartThreadPool = new SmartThreadPool();
                //var para = new AppSessionClosedEventArgs<TAppSession>(removedSession, e.Reason);
                //IWorkItemResult wir1 = smartThreadPool.QueueWorkItem(() => {
                //    OnAppSessionClosed(this, para);
                //});
        ...
        private int _OnAppSessionClosed(object sender, AppSessionClosedEventArgs<TAppSession> e) {
            return 0;
        }

        protected virtual void OnAppSessionClosed(object sender, AppSessionClosedEventArgs<TAppSession> e)
        {
            _OnAppSessionClosed(sender, e);
        }
        ...
        private void ClearIdleSession(object state)
        {
            if (System.Threading.Monitor.TryEnter(state))
            {
                try
                {
                    ...
                    var timeOutSessions = SessionSource.Where(s => s.Value.LastActiveTime <= timeOut).Select(s => s.Value).ToArray();
                    ////Tasks.Parallel.ForEach(timeOutSessions, s =>
                    ////    {
                    ...
                    int km = timeOutSessions.Length;
                    SmartThreadPool smartThreadPool = new SmartThreadPool();
                    IWorkItemResult[] tasks = new IWorkItemResult[km];
                    for (int k = 0; k < km; k++)  
                        tasks[k] = smartThreadPool.QueueWorkItem(new WorkItemCallback(this.DoClose), timeOutSessions[k]); 
        ...
        private object DoClose(object state)
        {
            ((TAppSession)state).Close();
            return 1;
        }
        ...
        /// <returns></returns>
        public override IEnumerable<TAppSession> GetSessions(System.Func<TAppSession, bool> critera)
        {
        ...
        public PerformanceData CollectPerformanceData(GlobalPerformanceData globalPerfData)
        {
            ...
            //////SmartThreadPool smartThreadPool = new SmartThreadPool(); 
            //////IWorkItemResult wir1 = smartThreadPool.QueueWorkItem(() => {
            //////    OnPerformanceDataCollected(globalPerfData, m_PerformanceData);
            //////});
            //////bool success = SmartThreadPool.WaitAll(new IWorkItemResult[] { wir1 });
            ////////if (success)  result1 = (int)wir1.Result;  
            //////smartThreadPool.Shutdown();
        ...
        protected override void Dispose(bool disposing)
        {
            ...
            if (disposing)
            {
                ...
                if (sessions.Length > 0)
                {
                    ...
                    SmartThreadPool smartThreadPool = new SmartThreadPool();
                    var tasks = new IWaitableResult[sessions.Length];
                    for (var i = 0; i < tasks.Length; i++) 
                        tasks[i] = smartThreadPool.QueueWorkItem(new WorkItemCallback(this.Session_Close), sessions[i].Value);                      
                    bool success = SmartThreadPool.WaitAll(tasks);
                    //if (success)  result1 = (int)wir1.Result;  
                    smartThreadPool.Shutdown();
                }
        ...
        #endregion
        private object Session_Close(object state)
        {
            ((TAppSession)state).Close(CloseReason.ServerShutdown);
            return null;
        }
